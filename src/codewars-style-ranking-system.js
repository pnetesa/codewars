...
class User {
  ...
  incProgress(rank) {
    ...
    const diff = (Math.sign(rank) !== Math.sign(this.rank)) ? Math.abs(rank - this.rank) - 1 : Math.abs(rank - this.rank);
    if (diff >= 1 && rank > this.rank) {
    ...
    if (this.progress > 100 && this.rank < 8) {
      do {
        this.rank = (this.rank + 1) || this.rank + 2;
